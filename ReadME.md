...
프레그먼트를 전환하는 방법에는 replace()와 add()가 있다. replace는 프레그먼트를 교체하므로, 이전에 있던 프레그먼트는 프레그먼트 메니저가 내부적으로 가지고 있는다. add는 해당 레이아웃에 겹쳐서 추가하는 기능이다. 이 기능을 사용하면 프레그먼트 show와 hide의 기능을 사용할 수 있다. 뷰에 마지막에 추가된게 제일 상위에 있듯이 show와 hide를 사용할경우 마지막에 추가된 프레그먼트가 가장 우선순위를 갖는다. 둘을 적절히 잘사용해서 구현해야한다. addToBackStack를 사용할경우 스택에 프레그먼트가 쌓이게 되고 popBackStack를 사용해서 하나씩 밖으로 꺼낼 수 있다. addToBackStack에 테그부분을 잘 달아주면 필요없는 화면을 한방에 없앨 수 도 있다. 하지만 replace와 함께사용할 경우 popBackStack() 시 같은 프레그먼트가 중복으로 불려지는 경우 앱이 죽을 수 있으므로 이둘은 조심해서 사용해야한다. 

확실히 액티비티보다 다루기 어려우며 많은 연습을 해야할 것 같다. 